...
{
    /// <summary>
    /// Generate will produce a chunk of C# code BaseClass for the AElf Contract. based on C++ original https://github.com/AElfProject/contract-plugin/blob/453bebfec0dd2fdcc06d86037055c80721d24e8a/src/contract_csharp_generator.cc#L422
    /// </summary>
    private static string GenerateContractBaseClass(ServiceDescriptor service)
    {
        ...
        {
            output.AppendLine($"public abstract {GetMethodReturnTypeServer(method)} {method.Name}({GetMethodRequestParamServer(method)}{GetMethodResponseStreamMaybe(method)});\n");
        }
        // out->Outdent();
        output.AppendLine("}\n");
        output.AppendLine("\n");
    ...
    private static void DepthFirstSearchForBase(ServiceDescriptor service,ref List<string> list,ref SortedSet<string> seen, Dictionary<string, ServiceDescriptor> allServices)
    {
        if (!seen.Add(service.File.Name))
        {
            return;
        }
    ...
    private static void DepthFirstSearch(ServiceDescriptor service,ref List<ServiceDescriptor> list,ref SortedSet<ServiceDescriptor> seen)
    {
        if (!seen.Add(service))
        {
            return;
